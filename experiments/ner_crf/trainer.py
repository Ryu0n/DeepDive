...
from kobert_ner import save_kobert_crf, load_kobert_crf
...
class Trainer(object):
    def __init__(self, args, train_dataset=None, dev_dataset=None, test_dataset=None):
        ...
        # Initialize pretrained KoBERT Classifier.
        if args.model_type in CUSTOM_MODEL_CLASSES.keys():
            self.model_class = CUSTOM_MODEL_CLASSES[args.model_type]
    ...
    def evaluate(self, mode, step):
        ...
        self.save_model(f1=result.get('f1'))
        return results

    def save_model(self, f1=None):

        if self.args.model_type in CUSTOM_MODEL_CLASSES.keys():
            torch.save(self.args, os.path.join(self.args.model_dir, 'training_args.bin'))
            save_kobert_crf(args=self.args, model=self.model, f1=f1)
    ...
    def load_model(self):
        if self.args.model_type in CUSTOM_MODEL_CLASSES.keys():
            self.model = load_kobert_crf(self.args)
            self.model.to(self.device)
