...
class LSHRetriever:
	def __init__(self, hash_tables: list, composite_hash_functions: list, threshold: float = 1.5):
	...
	def query(self, vector: np.ndarray):
		for i in range(len(self.hash_tables)):
			...
			if not retrieved_vectors:
				...
				continue
			print(f"Hash table {i}: Retrieved {len(retrieved_vectors)} vectors for hash value {hash_value}")
			for v in retrieved_vectors:
				distance = np.linalg.norm(vector - v)
				print(f"query vector: {vector}, retrieved vector: {v}, distance: {distance}")
...
if __name__ == "__main__":
	...
	hash_tables, composite_hash_functions = LSHPreprocessor(L=L, k=k, dim=dim).preprocess(dataset)
